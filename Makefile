...
fuzz_charll_no.o: fuzz_charll.cpp
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -DNO_FUZZ -c -o $@ fuzz_charll.cpp
...
fuzz-charll:	fuzz_charll.o simplecpp.o
	# TODO: use -stdlib=libc++ -lc++
	# CXX=clang++ CXXOPTS="-O2 -fno-omit-frame-pointer -g -gline-tables-only -DFUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION -fsanitize=address,undefined -fsanitize-address-use-after-scope -fno-sanitize=integer -fno-sanitize-recover=undefined" LIB_FUZZING_ENGINE="-fsanitize=fuzzer"
	$(CXX) $(LDFLAGS) $(CXXFLAGS) -o $@ $^ $(LIB_FUZZING_ENGINE)

no-fuzz-charll:	fuzz_charll_no.o simplecpp.o
...
clean:
	rm -f testrunner fuzz-charll no-fuzz-charll simplecpp *.o
